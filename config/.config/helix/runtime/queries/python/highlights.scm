...
; Docstrings
; -------
; A docstring is a string literal that is the FIRST statement of a module, class
; body, or function body. Re-scope only those to `comment.block.documentation`
; so they can be themed apart from ordinary triple-quoted strings (which stay
; `@string`). This block is intentionally last: Helix uses last-match-wins, so
; it overrides the generic `(string) @string` rule above for these positions only.

(module . (expression_statement (string) @comment.block.documentation))

(class_definition
  body: (block . (expression_statement (string) @comment.block.documentation)))
